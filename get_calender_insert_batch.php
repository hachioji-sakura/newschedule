<?php
//echo "Program Start.\n";
// this routine load google calender data into tbl_schedule_onetime.
// Syntax: php load_calender_data.php [ onetime or repeat] YEAROFSTART MONTHOFSTART DAYOFSTART YEAROFEND MONTHOFEND DAYOFEND

ini_set( 'display_errors', 0 );

$request_repeat = $argv[1];

$request_min_year = $argv[2];

$request_min_month = $argv[3];

$request_min_day = $argv[4];

$request_max_year = $argv[5];

$request_max_month = $argv[6];

$request_max_day = $argv[7];

if ($request_min_month > 12 or $request_max_month > 12 ) {
	echo('min_month or max_month must be less than 13.');
	$err_flag = true;
	goto exit_label;
}
if ($request_min_year < 2019 or $request_max_year < 2019 ) {
	echo('min_year or max_year must be bigger than 2018.');
	$err_flag = true;
	goto exit_label;
}

if ($request_max_day == '') {
	echo('Syntax error: php get_calender_insert.php [onetime|repeat] STARTYEAR STARTMONTH STARTDAY ENDYEAR ENDMONTH ENDDAY');
	$err_flag = true;
	goto exit_label;
}
if ($request_min_day > 31 or $request_max_day > 31 ) {
	echo('min_day or max_day must be less than 31.');
	$err_flag = true;
	goto exit_label;
}

$request_target_calender = $argv[8];
if ($request_target_calender  and $request_target_calender !='hachioji2f') {
	echo('target calender should be hachioji2f if specified.');
	$err_flag = true;
	goto exit_label;
}
...
define(CONST_TESTSCHOOL, '塾_テスト校');
...
foreach ($calList->getItems() as $calender) {
	...
	if ($request_target_calender) {
				// target_calenderが指定されたらそのカレンダーだけロードする
		if ($calender['summary'] !== '塾_八王子_2F') {
...
var_dump($calender['summary']);
	...
	$place_id = 0;
        foreach ($floor_list as $floor) {
               if (mb_strpos($floor["calendar_name"],$calender['summary'])!== FALSE) {
                      $place_id = (int) $floor["no"];
                      break;
               } 
        } // for each $floor
	...
	$minyear = $request_min_year;
	$minmonth = $request_min_month;
	$minday = $request_min_day;
	$maxyear = $request_max_year;
	$maxmonth = $request_max_month;
	$maxday = $request_max_day;
        $objMinDateTime->setTimeStamp(mktime(0, 0, 0, $minmonth, $minday, $minyear));
        $objMaxDateTime->setTimeStamp(mktime(23, 59, 59, $maxmonth, $maxday, $maxyear));
...
//		if (preg_match("/(\(仮\)|\(not\s*defin|\(not\s*confirm|temporary)/i", $tmp_event_summary, $matches, PREG_OFFSET_CAPTURE) == 1) {
		if (preg_match("/(\(仮|\(not\s*defin|\(not\s*confirm|temporary)/i", $tmp_event_summary, $matches, PREG_OFFSET_CAPTURE) == 1) {
			$temporary = 1;
			$tmp_event_summary = str_replace(array("(仮)"), array(""), trim($tmp_event_summary));
			$tmp_event_summary = str_replace(array("(仮"), array(""), trim($tmp_event_summary));
		}
...
//			$absent_flag = 0;
			...
			if ($request_repeat != 'repeat'){	
				...
				// くりかえしスケジュール	
				$result = insert_calender_repeatevent($dbh,
								$event,
								$start_timestamp,
								$end_timestamp,
								$repetition_id,
...
//			$place_id = $event_param[19];
			...
			$temp_name = $event_param[3];		// setting trial member's name.
			if ($trial_id !== ' '){			// This is a trial lesson.
				$trial_num = $event_param[38];	// setting trial student's grade.
			} else {
				$trial_num = 0;
			}
...
function get_family(&$db, $family_data) {
        ...
        if (preg_match("/当日|Today/", $family_data, $matches, PREG_OFFSET_CAPTURE) == 1) {
	...
	$grdade = '';
        ...
        $student_array = array("id"=>"", 
				...
				"alternative_flag"=>$alternative_flag,
                   		"cal_name"=>$cal_name, 
				"absent1_num"=>$absent1_num, 
				"absent2_num"=>$absent2_num, 
				"trial_num"=>$trial_num,
				"attendance_data"=>$family_data,
				"grade"=>$grade, 
				"work"=>$work, 
...
function get_student(&$db, $student_data, $trial_flag) {
        ...
        foreach ($words as $key => $word) {
                $word = trim($word);
                if (mb_strpos($word, "小１") !== FALSE )  { $word = "小1"; }
                if (mb_strpos($word, "小２") !== FALSE )  { $word = "小2"; }
                if (mb_strpos($word, "小３") !== FALSE )  { $word = "小3"; }
                if (mb_strpos($word, "小４") !== FALSE )  { $word = "小4"; }
                if (mb_strpos($word, "小５") !== FALSE )  { $word = "小5"; }
                if (mb_strpos($word, "小６") !== FALSE )  { $word = "小6"; }
                if (mb_strpos($word, "中１") !== FALSE )  { $word = "中1"; }
                if (mb_strpos($word, "中２") !== FALSE )  { $word = "中2"; }
                if (mb_strpos($word, "中３") !== FALSE )  { $word = "中3"; }
                if (mb_strpos($word, "高１") !== FALSE )  { $word = "高1"; }
                if (mb_strpos($word, "高２") !== FALSE )  { $word = "高2"; }
                if (mb_strpos($word, "高３") !== FALSE )  { $word = "高3"; }
			...
			if (strpos($word,$str)!==false) { $grade=$key1; }
				...
				if (preg_match("/\(仮/i", $tmp_cal_name, $matches, PREG_OFFSET_CAPTURE) == 1) {
					$tmp_cal_name = str_replace(array("(仮)"), array(""), trim($tmp_cal_name));
					$tmp_cal_name = str_replace(array("(仮"), array(""), trim($tmp_cal_name));
		}
//var_dump($tmp_cal_name);
                        }
			$member_no = -1;		// initialization
                        if ( $trial_flag != "0" ) { $tmp_cal_name = "体験生徒";}
		
                        foreach ($member_list as $no => $member) {
                    		$tmp_db_name = str_replace(array("　", " "), array(" ", ""), $member['name']);// 半角スペースも除去
//var_dump($tmp_db_name);
        	...
        	else if (mb_strpos($word, CONST_ABSENTLATE) !== FALSE || mb_strpos($word, CONST_ABSENTLATE_ENG) !== FALSE ) {
                	// 当日
                        $cancel_reason = CONST_ABSENTLATE;
                }
        }       // End:foreach ($words as $key => $word)
		// $student_arrayに"work" と”cancel_reason"を追加　by T.Kobayashi
        $student_array = array("id"=>"", 
				"no"=>$member_no, 
				"kind"=>"student", 
				"absent_flag"=>$absent_flag, 
...
//       	$place_id = 0;
//        foreach ($floor_list as $floor) {
//       		foreach ($floor["calendar_name_list"] as $calendar_name) {
//       	        	if ($event["summary"] == $calendar_name) {
//       	        		 $place_id = $floor["no"];
//       	                   break;
//       		         } else if (preg_match("/^(.+)?".$calendar_name."$/", $event["summary"]) === 1) {
//       		 		$place_id = $floor["no"];
//               	        	break;
//               		} // else if 
//       		} // foreach
...
// $param_array[40]に$workを追加 , $event[summary]に変更　by T.Kobayashi
		...
		if (!$member["work"]){ $member["work"] = $work; }	// if empty then set default work.
				...
				date("n", $start_timestamp), 
				date("j", $start_timestamp),
				$start_timestamp, 
				date("H", $start_timestamp), 
				date("i", $start_timestamp),
				$end_timestamp, 
				date("H", $end_timestamp), 
				date("i", $end_timestamp), 
				$tmp_diff_hours, 
				$lesson_id, 
				$subject_id, 
				$course_id, 
				...
				$member["absent_flag"], 
				$member["trial_flag"], 
				$member["interview_flag"],
				$member["alternative_flag"],
				$member["absent1_num"], 
				$member["absent2_num"], 
				$member["trial_num"], 
				null, 
